...
// Para obter todos os itens, lemos o arquivo `users.json, realizamos o parsing e retornamos o resultado
const getAll = async () => fs.readFile(DATA_PATH, 'utf-8').then(JSON.parse);

// Para alterar o arquivo `users.json`, recevemos um array, convertemos o array em JSON e escrevemos o resultado no disco
...
const create = (username, password, admin) =>
  getAll() // Buscamos todos os itens
    .then((users) => {
      users.push({ username, password, admin }); // Adicionamos o item novo
      return users;
    })
    .then(writeAll); // Armazenamos o Array no disco
